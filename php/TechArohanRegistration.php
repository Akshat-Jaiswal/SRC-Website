...
//First Register Team	
	$sql="INSERT INTO `participatingteams` (`TeamName`,`Event`,`RegistrationDate`) VALUES ( '$teamName' ,'TechArohan','$dt' );";	
	// query database
	...
	if(!$result){	
		...
		$msg="Team Registration Successful For Sarthi";
		sendCard($email,$msg);
	}
//now Register Members	
//First register primary member
...
//member 3 Registration
	if(isset($_POST['name4']) && isset($_POST['email4']) && isset($_POST['contact4']) && isset($_POST['address4']) && isset($_POST['college4'])){
		$user=mysql_fix_string($_POST['name4']);
		$email=mysql_fix_string($_POST['email4']);
		$contact=mysql_fix_string($_POST['contact4']);
		$address=mysql_fix_string($_POST['address4']);
		$college=mysql_fix_string($_POST['college4']);
		$sql="INSERT INTO `participants` (`TeamName`,`Name`,`Email`,`Contact`,`Address`,`College`) VALUES ( '$teamName' ,'$user','$email','$contact','$address','$college');";	
		$result=mysqli_query($con,$sql);
		if(!$result){	
			// in case of failure
...
//member 5 Registration
	if(isset($_POST['name5']) && isset($_POST['email5']) && isset($_POST['contact5']) && isset($_POST['address5']) && isset($_POST['college5'])){
		$user=mysql_fix_string($_POST['name5']);
		$email=mysql_fix_string($_POST['email5']);
		$contact=mysql_fix_string($_POST['contact5']);
		$address=mysql_fix_string($_POST['address5']);
		$college=mysql_fix_string($_POST['college5']);
		$sql="INSERT INTO `participants` (`TeamName`,`Name`,`Email`,`Contact`,`Address`,`College`) VALUES ( '$teamName' ,'$user','$email','$contact','$address','$college');";	
		$result=mysqli_query($con,$sql);
		if(!$result){	
			// in case of failure
