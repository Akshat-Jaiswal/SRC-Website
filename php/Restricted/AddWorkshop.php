...
if(isset($_POST['title']) && isset($_POST['description']) && isset($_FILES['imageURL'])  ){
	...
	$dt=mysqli_real_escape_string($con,$_POST['wdate']);
	//$user='akshatjaiswal1995@gmail.com';
	$user=$_SESSION['username'];
	//get filename
	// get the directory for storing file
	//$root=DirConfig::ROOT;
	$directory="../../".DirConfig::WORKSHOPS;
	//get file extension	
...
// connect to database
	$imageURL=DirConfig::ROOT.DirConfig::WORKSHOPS.$fname;	
	if($_FILES['imageURL']['error']>0){
		$response=array('code' => '5', 'data' => 'ERROR while uploading file File not Found');
		die(json_encode($response));
	}
	move_uploaded_file($_FILES['imageURL']['tmp_name'],$directory.$fname);
// query for inserting into posts table
	$sql = "INSERT INTO `workshops`(`Title`, `Description`,`imageURL`, `Wdate`,`link`) VALUES ('$title','$description','$imageURL','$dt','#')";
